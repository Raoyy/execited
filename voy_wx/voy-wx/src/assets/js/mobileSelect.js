...
(function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
		    ...
		    _this.ensureBtn.addEventListener('click',function(){               //点击确定
				_this.mobileSelect.classList.remove('mobileSelect-show');
				var tempValue ='';
		    	...
		    	var oLi = _this.trigger.parentNode;
		...
		setSelectedColor: function(){
			var arrIndex = this.getIndexArr();
			var ulLength = arrIndex.length;
			$(".mobileSelect .wheels .wheel").find("li").each(function(){
				$(this).removeClass("selected");
			});
			$(".mobileSelect .wheels .wheel").eq(ulLength-1).find("li").eq(arrIndex[ulLength-1]).addClass("selected");
		},
		...
		checkArrDeep: function (parent) { 
			...
			if(parent){
				if (_this.keyMap.childs in parent && parent[_this.keyMap.childs].length > 0) {
					_this.displayJson.push(_this.generateArrData(parent[_this.keyMap.childs])); //����ӽڵ�����
					_this.checkArrDeep(parent[_this.keyMap.childs][0]);//�����һ���ӽڵ�
				}
			}
		},
		...
		updateWheel: function(sliderIndex, data){
			...
			var tempHTML='';
	    	if(_this.cascade){
	    		console.error('������ʽ��֧��updateWheel(),��ʹ��updateWheels()����������Դ');
				return false;
	    	}
