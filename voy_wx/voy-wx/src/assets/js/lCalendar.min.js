	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			if (this.trigger.getAttribute("data-lcalendar") != null) {
				...
				this.minM = ~~minArr[1];
				this.minD = ~~minArr[2];
				if(this.trigger.getAttribute("now")){
				   var date = new Date();
					this.maxY = date.getYear() + 1900;
					this.maxM = date.getMonth()+1;
					this.maxD = date.getDate() ;
				}else{
		...
		bindEvent: function(type) {
			...
			//呼出星期插件
			 function popupWeek(e){
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'<div class="time_roll">' +
					'<div>' +
					'<div class="gear week" data-datetype="week"></div>' +
					'<div class="date_grid">' +
					'<div style="left:35%">星期</div>' +
					'</div>' +
					'</div>' +
					//'<div>' +
					/*'<div class="gear time_mm" data-datetype="time_mm"></div>' +
				...
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
				lcalendar_finish.addEventListener('touchstart', finishMobileDateWeek);
				var week = _self.gearDate.querySelector(".week");
				
				week.addEventListener('touchstart', gearTouchStart);
				week.addEventListener('touchmove', gearTouchMove);
				week.addEventListener('touchend', gearTouchEnd);
			 }
			//初始化星期插件默认值
			function weekCtrlInit() {
				...
				//alert(e.ss);   测试用
				console.log(e.dd);
				_self.gearDate.querySelector(".week").setAttribute("val", e.dd);
				setWeekGearTooth();
			}
			//重置星期节点个数
			function setWeekGearTooth() {
				var week = _self.gearDate.querySelector(".week");
				if (week && week.getAttribute("val")) {
					var i = "";
					var hhVal = parseInt(week.getAttribute("val"));
					for (var g = 1; g <= 7; g++) {
						i += "<div class='tooth'>" + g + "</div>";
					}
					week.innerHTML = i;
					week.style["-webkit-transform"] = 'translate3d(0,' + (8 - hhVal * 2) + 'em,0)';
					week.setAttribute('top', 8 - hhVal * 2 + 'em');
				} else {
					return
				}
			}
			...
			function finishMobileDateWeek(e) {
				console.log(_self.gearDate.querySelector(".week").getAttribute("val"));
				var week = parseInt(Math.round(_self.gearDate.querySelector(".week").getAttribute("val")));
				console.log(week);
				_self.trigger.value = week;
				closeMobileCalendar(e);
			}
